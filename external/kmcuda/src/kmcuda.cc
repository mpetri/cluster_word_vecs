...
KMCUDAResult kmeans_cuda(
  ...
  if (origin_devi < 0) {
    if (device_ptrs < 0) {
	std::cout << "host address: " << std::hex << centroids << std::endl;
	std::cout << "device address: " << std::hex << device_centroids[devs.back()].get() << std::endl;
	std::cout << "size: " << std::dec << centroids_size * sizeof(float) << std::endl;
      CUCH(cudaMemcpy(centroids, device_centroids[devs.back()].get(),
                      centroids_size * sizeof(float), cudaMemcpyDeviceToHost),
           kmcudaMemoryCopyError);
	std::cout << "host address: " << std::hex << assignments << std::endl;
	std::cout << "device address: " << std::hex << device_assignments[devs.back()].get() << std::endl;
	std::cout << "size: " << std::dec << samples_size * sizeof(float) << std::endl;
